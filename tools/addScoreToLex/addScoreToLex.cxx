...
main(int argc, char const** argv)
{
  ...
  try {
    ...
    if (!options.hasOption("defaultFreq") && options.getArgumentCount() != 1) {
      std::cerr
        << "Use like: addScoreToLex --defaultScore=n words_with_scores.mdic  < "
           "somelex.lex > somelex_with_scores.lex"
        << std::endl;
      exit(1);
    }
    ...
    if (options.hasOption("defaultFreq")) {
      mdic.loadFromFile("/mounts/data/proj/kristof/ARBEITSGRUPPE/ressourcen/"
                        "freqListen/web_in_a_box/web_liste_gefiltert.frq.mdic");
    } else {
    ...
    // this way of reading will SKIP THE LAST LINE if it is not properly
    // terminated by \n
