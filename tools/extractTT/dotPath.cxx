...
main(int argc, const char** argv)
{
  ...
  if (options.getArgumentCount() != 1) {
    std::wcerr
      ...
      << "dotPath reads one word from stdin and prints dot code for all states "
         "which are on the path of that input word in the automaton."
      << std::endl
      << std::endl
      << std::endl;
  ...
  try {
    ...
    while (st.isValid() && c != word.end()) {
      for (wchar_t const* suso = st.getSusoString(); *suso; ++suso) {
        ...
        std::wcout << st.getStateID() << "->"
                   << st.getTransTarget(*suso).getStateID() << "[" << color
                   << "label=\"" << *suso << "\"]; // DOTCODE" << std::endl;
      }
