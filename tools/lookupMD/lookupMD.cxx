...
public:
  ...
  virtual void receive(const wchar_t*, int, int) override { found_ = true; }
  virtual void reset() override { found_ = false; }
...
main(int argc, char const** argv)
{
  ...
  try {
    ...
    csl::FBDic<>* fbdic =
      0; // this one is loaded in case a fbdic is passed to the program
    ...
    if (k > 0) {
      ...
      // In case a .fbdic file is passed, open it and use the FWDic
               ...
               options.getArgument(0).substr(options.getArgument(0).size() -
                                             5) == "fbdic") {
    ...
    while (std::getline(std::wcin, query).good()) {
      ...
      if (query.length() > Global::lengthOfLongStr) {
        throw exceptions::badInput(
          "csl::lookupMD: Maximum length of input line violated (set by "
