...
main(int argc, const char** argv)
{
  ...
  if (options.getArgumentCount() != 1) {
    std::wcerr << std::endl
               ...
               << "--annType=float\tto indicate that the MinDic has "
                  "annotations of type 'float' (instead of default 'int')"
               << std::endl
               << std::endl;
  ...
  try {
    ...
    if (options.hasOption("annType") &&
        (options.getOption("annType") == "float")) {
      csl::MinDic<float> t;
