...
DictSearch::~DictSearch()
{
  ...
  for (std::vector<iDictModule*>::iterator dm = internalDictModules_.begin();
       dm != internalDictModules_.end();
       ++dm) {
    delete (*dm);
  }
  if (vaam_)
...
void
DictSearch::readConfiguration(char const* configFile)
{
...
void
DictSearch::readConfiguration(INIConfig const& iniConf)
{

  // of all sections in iniConf, find those which start with "dict_" and have a
  // key-value pair "active = true"
  std::vector<std::string> activeDictionaries;
  for (INIConfig::SectionIterator section = iniConf.sectionsBegin();
       section != iniConf.sectionsEnd();
       ++section) {
    if ((std::string(*section).substr(0, 5) == "dict_") &&
        iniConf.hasKey(std::string(*section) + ":active") &&
        (strcmp(iniConf.getstring(std::string(*section) + ":active"), "true") ==
         0)) {
      activeDictionaries.push_back(*section);
    }
  }

  for (std::vector<std::string>::const_iterator it = activeDictionaries.begin();
       it != activeDictionaries.end();
       ++it) {
    ...
    if (iniConf.getstring(*it + ":dict_type") == std::string("simple")) {
      ...
      DictModule& dm =
        addDictModule(wideName, iniConf.getstring(*it + ":path"), cascadeRank);

      dm.setMaxNrOfPatterns(iniConf.getint(*it + ":histPatterns"));

      if (std::string(iniConf.getstring(*it + ":ocrErrors")) ==
          "LENGTH_SENSITIVE") {
      ...
      if (iniConf.hasKey(*it + ":caseMode")) {
        ...
        } else if (std::string("toLower") ==
                   iniConf.getstring(*it + ":caseMode")) {
          dm.setCaseMode(Global::toLower);
        } else if (std::string("restoreCase") ==
                   iniConf.getstring(*it + ":caseMode")) {
          dm.setCaseMode(Global::restoreCase);
        } else {
          throw exceptions::cslException(
            "csl::DictSearch::readConfiguration: unknown Global::caseMode");
        }
      }
    }
    // annotated dictionaries
    else if (iniConf.getstring(*it + ":dict_type") ==
             std::string("annotated")) {
      AnnotatedDictModule* sdm = new AnnotatedDictModule(
        *this, wideName, iniConf.getstring(*it + ":path"), cascadeRank);
      if (std::string(iniConf.getstring(*it + ":ocrErrors")) ==
          "LENGTH_SENSITIVE") {
    ...
    // adaptive dictionaries
      ...
      std::unique_ptr<AdaptiveLex> adm(new AdaptiveLex(
        OCRCorrection::Utils::utf8(*it), cascadeRank, ocr_errors));
      addDictModule(*adm.release());
    } else {
      throw exceptions::cslException("csl::DictSearch::readConfiguration: "
                                     "unknown dict_type for dictionary " +
                                     *it);
    }
...
void
DictSearch::initHypothetic(char const* patternFile)
{
  if (vaam_ || val_)
    throw exceptions::LogicalError(
      "csl::DictSearch: Tried to initialise Vaam twice.");
...
DictSearch::findDictModule(const std::wstring& name) const
{
  ...
  auto find = [&name](const Val& val) { return val.second->getName() == name; };
...
DictSearch::DictModule&
DictSearch::addDictModule(std::wstring const& name,
                          std::string const& dicFile,
                          size_t cascadeRank)
{
...
DictSearch::AnnotatedDictModule&
DictSearch::addAnnotatedDictModule(AnnotatedDictModule* newDM)
{
  internalDictModules_.push_back(newDM);
  allDictModules_.insert(std::make_pair(newDM->getCascadeRank(), newDM));
  return *newDM;
...
void
DictSearch::addExternalDictModule(iDictModule& extModule)
{
  externalDictModules_.push_back(&extModule);
  allDictModules_.insert(
    std::make_pair(extModule.getCascadeRank(), &extModule));
}

bool
DictSearch::hasDictModules() const
{
  return (internalDictModules_.size() + externalDictModules_.size() != 0);
}

bool
DictSearch::hasHypothetic() const
{
  return (vaam_);
}

bool
DictSearch::query(std::wstring const& query, iResultReceiver* answers)
{
  bool foundAnswers = false;

  size_t cascadeRank = 0;
  for (std::multimap<size_t, iDictModule*>::const_iterator dm =
         allDictModules_.begin();
    ...
    // 		std::wcerr << "csl::DictSearch::query: caught exception: " <<
    // exc.what() << std::endl;
    // 	    }
  }
  return foundAnswers;
...
void
DictSearch::enableUnknownVirtualLex()
{
