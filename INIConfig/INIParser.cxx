...
iniparser_dump_ini(dictionary* d, FILE* f)
{
  ...
  for (i = 0; i < nsec; i++) {
    ...
    fprintf(f, "\n[%s]\n", secname);
    snprintf(keym, (ASCIILINESZ + 1) * 2 + 1, "%s:", secname);
    for (j = 0; j < d->size; j++) {
      if (d->key[j] == NULL)
        continue;
...
/*--------------------------------------------------------------------------*/
int
iniparser_find_entry(dictionary* ini, char const* entry)
{
  int found = 0;
...
/*--------------------------------------------------------------------------*/
int
iniparser_set(dictionary* ini, char const* entry, char const* val)
{
  ...
  while (((startPos = stdValue.find("${", startPos)) != std::string::npos) &&
    ...
    if ((endPos = stdValue.find("}", startPos + 2)) != std::string::npos) {
      std::string variable =
        stdValue.substr(startPos + 2, endPos - startPos - 2);
      std::wstring wideVariable = OCRCorrection::Utils::utf8(variable);
  ...
  std::wstring wide = OCRCorrection::Utils::utf8(stdValue);
...
iniparser_load(const char* ininame)
{
  ...
  std::ifstream in;
  // utf8 locale. make sure punctuation is in basic style, i.e., a '.' for
  // decimals
  ...
  while (in.good()) {
    ...
    switch (iniparser_line(line, section, key, val)) {
      ...
      case LINE_VALUE:
        snprintf(tmp, (ASCIILINESZ + 1) * 2 + 1, "%s:%s", section, key);
