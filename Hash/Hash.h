 ...
 * designed to be used for hashing suso-strings for fast traversable
 * dictionaries -
 *
 * Inserted strings are stored in a buffer here in Hash, the hash table stores
...
public:
   ...
   * constructor
   *
   * @param[in/out] keyBuffer a reference to a c-string, where the keys are
   * stored.
   * @param[in] estimatedNrOfKeys an estimation that helps to choose the right
   * size of the hashtable
   * @param[in/out] lengthOfKeyBuffer
   * @todo think about the heuristics concerning the buffer initialization size
   */
  inline Hash(size_t estimatedNrOfKeys,
              charType_t*& keyBuffer,
              size_t& lengthOfKeyBuffer);
...
private:
  ...
  inline charType_t* strncpyTemplate(charType_t* w1,
                                     const charType_t* w2,
                                     size_t n) const;
...
inline Hash<charType_t>::Hash(size_t estimatedNrOfKeys,
                              charType_t*& keyBuffer,
                              size_t& lengthOfKeyBuffer)
  ...
  if (keyBuffer_ == 0) {
    if (lengthOfKeyBuffer_ != 0) {
      throw exceptions::LogicalError(
        "Hash-Constructor: non-compatible buffer / size");
    }
...
template<class charType_t>
inline size_t
Hash<charType_t>::find(const charType_t* key) const
{
...
template<class charType_t>
inline size_t
Hash<charType_t>::findOrInsert(const charType_t* key)
{
  uint_t slot = getHashCode(key);
  while (table_.at(slot) != 0 && // hash-slot non-empty
         strcmpTemplate(key, getStringAt(table_.at(slot)))) { // no match
  ...
  if (table_.at(slot) == 0) { // key wasn't found
    ...
    // std::wcout << "Create: " << key << " at slot " << slot << ", buf_offset "
    // << table_.at( slot ) << std::endl;
  } else {
    // std::wcout << "Found: " << key << " at slot " << slot << ", buf_offset "
    // << table_.at( slot ) << std::endl;
  }
  return table_.at(slot);
}
...
template<class charType_t>
inline const charType_t*
Hash<charType_t>::getStringAt(size_t offset) const
{
...
template<class charType_t>
inline charType_t*
Hash<charType_t>::getStringAt(size_t offset)
{
  return (keyBuffer_ + offset);
}
...
template<class charType_t>
inline size_t
Hash<charType_t>::getTableSize() const
{
  return table_.size();
}
...
template<class charType_t>
inline size_t
Hash<charType_t>::getLengthOfKeyStrings() const
{
...
template<>
inline size_t
Hash<uchar>::getHashCode(const charType_t* str) const
{
  size_t h = 0;
  while (*str) {
...
template<>
inline size_t
Hash<wchar_t>::getHashCode(const charType_t* str) const
{
  uint_t h = 0;
  while (*str) {
...
Hash<charType_t>::getTable() const
{
  return table_;
}
...
template<class charType_t>
inline void
Hash<charType_t>::reallocKeyBuffer(size_t newSize)
{
...
template<>
inline int
Hash<wchar_t>::strcmpTemplate(const charType_t* w1, const charType_t* w2) const
{
  return wcscmp(w1, w2);
}
...
template<>
inline wchar_t*
Hash<wchar_t>::strncpyTemplate(charType_t* w1,
                               const charType_t* w2,
                               size_t n) const
{
...
template<>
inline size_t
Hash<wchar_t>::strlenTemplate(const charType_t* w) const
{
  return wcslen(w);
}
...
template<>
inline int
Hash<uchar>::strcmpTemplate(const charType_t* w1, const charType_t* w2) const
{
...
template<>
inline uchar*
Hash<uchar>::strncpyTemplate(charType_t* w1,
                             const charType_t* w2,
                             size_t n) const
{
...
template<>
inline size_t
Hash<uchar>::strlenTemplate(const charType_t* w) const
{
