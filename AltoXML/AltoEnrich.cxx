...
// ---------------------------------------------------------------------------
	...
	for( std::vector< std::string >::iterator entry = dirEntries.begin(); entry != dirEntries.end(); ++entry ) {
		...
		std::wcout << "Parse " << Utils::utf8(inFile) << std::endl;
		...
		++slavePageIterator_;
		std::wcerr << "Successfully wrote " << Utils::utf8(outFile) << std::endl;
	...
	std::wstring wideName(Utils::utf8(xmlReaderHelper_.name()));
	...
	if( xmlReaderHelper_.name() == "String" ) {
	    ...
	    *ostream_ << "<" << wideName;
        ...
        value_str = Utils::utf8(value_cstr);
...
//csl::CSLLocale::string2wstring( key_cstr, key_str );
	...
	std::wstring wideName(Utils::utf8(name));
