...
/**
 * @brief Read the {@link profiler_manual Profiler manual} to find out more
 * about the purpose and use of the Profiler. Class Profiler analyzes an ocr-ed
 * historical text and draws a number of conclusions and to return a profile of
 * the text, regarding mainly:
 *        - estimated probabilities for frequent historical orthographical
 * patterns
...
#include "./Profiler_Token.h"
  ...
  /**
   * @brief For now, Document_t is a vector of custom tokens of type
   * Profiler_Token
   *
   * At a later step it will make sense to solve this using
   * OCRCorrection::Document.
   */
...
#include "./HTMLWriter.h"
  ...
  /**
   * @brief sort operator to sort two {@Profiler_Interpretation}s according to
   * their combined-probability values.
   */
  class CandSort
  {
  public:
    bool operator()(Profiler_Interpretation const& first,
                    Profiler_Interpretation const& second)
    {
...
public:
  ...
  bool adaptive() const noexcept { return config_.adaptive_; }
  ...
  bool types() const noexcept { return config_.types_; }
  void setTypes(bool types) noexcept { config_.types_ = types; }

  /**
   * @brief enable virtual unknown dictionary
   */
  void enableUnknownVirtualLex() { dictSearch_.enableUnknownVirtualLex(); }
  ...
  inline void setHTMLOutFile(std::string const& htmlFile)
  {
    ...
    std::locale loc =
      std::locale(std::locale(""), new std::numpunct<wchar_t>());
    if (!htmlStream_->good()) {
      delete (htmlStream_);
      htmlStream_ = 0;
      throw OCRCException(
        std::string("OCRC::Profiler:: setHTMLOutFile: Can not write to ") +
        htmlFile);
    }
  }
...
protected:
   ...
   */
  csl::DictSearch& getDictSearch() { return dictSearch_; }
...
private:
  ...
  void calculateAdaptiveCandidateSet(const Token& t,
                                     csl::DictSearch::CandidateSet& candidates);
  void calculateNonAdaptiveCandidateSet(
    const Token& t,
    csl::DictSearch::CandidateSet& candidates);
  void createCandidatesWithCorrection(
    const Token& t,
    csl::DictSearch::CandidateSet& candidates);
   ...
   *
   * Both the class' naming and its mere existence as an entity separate from
   * the profiler are a bit outdated. So the Profiler class itself would
   * probably be a better place for its functionality.
   */
  FrequencyList freqList_;

  /**
   * @brief csl::DictSearch handles all the variant- and error-tolerant
   * dictionary lookups.
   */
  csl::DictSearch dictSearch_;

  /**
   * @brief Holds all external DictModules that were added to the DictSearch
   * object. We need to collect these in order to destrroy them safely.
   */
  std::vector<csl::DictSearch::iDictModule*> externalDictModules_;

  /**
   * @brief In {@link doIteration} for each token the candidates (appearing as
   * csl::DictSearch::Interpretation) are copied into this vector of {@link
   * Profiler_Interpretation}s.
   */
  std::vector<Profiler_Interpretation> candidates_;

  /**
   * @brief This module handles the computation of the ocrInstructions via
   * dynamic programming.
   */
  csl::ComputeInstruction instructionComputer_;

  /**
   * @brief During a run through the text, histCounter_ accumulates the
   * necessary statistics related to historical variant patterns.
   */
  PatternCounter histCounter_;

  /**
   * @brief During a run through the text, histCounter_ accumulates the
   * necessary statistics related to ocr error patterns.
   */
  ...
  /**
   * @brief This is a simple associative container to collect a type-frequency
   * list of all ocr-ed tokens
   */
  ...
  /**
   * @brief Matches each ocr pattern to a list of all types that detect this
   * pattern
   */
  std::map<csl::Pattern, std::vector<std::pair<std::wstring, std::wstring>>>
    ocrPatterns2Types_;
   ...
   */
  static bool sortBySecond(std::pair<csl::Pattern, double> const& a,
                           std::pair<csl::Pattern, double> const& b)
  {
  ...
  public:
    /**
     * @brief The constructor initializes the parameters with zero-values that
     * will let the profiler do nothing. So you are bound to explicitly set
     *        values.
     */
    Configuration()
      : nrOfIterations_(0)
    ...
    /**
     * @brief This number set to n means that only those hist patterns are kept
     * which are estimated to appear at least n times per 1000 profiled tokens
     */
    double patternCutoff_hist_;

    /**
     * @brief This number set to n means that only those ocr patterns are kept
     * which are estimated to appear at least n times per 1000 profiled tokens
     */
    double patternCutoff_ocr_;

    /**
     * @brief In the first iteration, all ocr patterns start with this uniform
     * probability
     */
    ...
    /**
     * @brief this bool value decides if, before setting new hist pattern
     * probabilities, the old list of probabilities is emptied. If this value is
     * false, all old probabilities are kept and updated only if a new value was
     * determined.
     */
    bool resetHistPatternProbabilities_;

    /**
     * @brief this bool value decides if, before setting new ocr pattern
     * probabilities, the old list of probabilities is emptied. If this value is
     * false, all old probabilities are kept and updated only if a new value was
     * determined.
     */
    ...
    void print(std::wostream& os = std::wcout)
    {
      os << "number of iterations:           " << nrOfIterations_ << std::endl
         ...
         << std::endl
         << "ocrPatternStartProb_:           " << ocrPatternStartProb_
         << std::endl
         << std::endl
         << "patternCutoff_hist_:            " << patternCutoff_hist_
         << std::endl
         << "patternCutoff_ocr_:             " << patternCutoff_ocr_
         << std::endl
         << std::endl
         << "histPatternSmoothingProb_:      " << histPatternSmoothingProb_
         << std::endl
         << "ocrPatternSmoothingProb_:       " << ocrPatternSmoothingProb_
         << std::endl
         << std::endl
         << "resetHistPatternProbabilities_: " << resetHistPatternProbabilities_
         << std::endl
         << "resetOCRPatternProbabilities_:  " << resetOCRPatternProbabilities_
         << std::endl
         << std::endl
         << "donttouch_hyphenation_:         " << donttouch_hyphenation_
         << std::endl
         << "donttouch_lineborders_:         " << donttouch_lineborders_
         << std::endl
         << std::endl
         << "writeAdaptiveDictionary:        " << writeAdaptiveDictionary_
         << std::endl
         << "adaptiveDictionaryPath:         "
         << Utils::utf8(adaptiveDictionaryPath_) << std::endl
