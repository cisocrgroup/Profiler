...
namespace OCRCorrection {
	...
	if( errno == EILSEQ ) { // catch encoding error
	    std::wcerr << "OCRC::TXTReader::parse: Encoding error in input sequence." << std::endl;
	    exit( 1 );
	    //throw exceptions::badInput( "MinDic::compileDic: Encoding error in input sequence." );
	}

	std::wcerr<<"OCRC::TXTReader::parse: Finished, "<< document->getNrOfTokens() << " tokens." << std::endl;
